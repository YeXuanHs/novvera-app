...
Future<List<FavoriteItem>> updateBooksInfo(String folder) async {
  var books = LocalFavoritesManager().getFolderBooks(folder);
  ...
  Future<void> updateSingleComic(int index) async {
    ...
    while (true) {
      try {
        ...
        LocalFavoritesManager().updateInfo(folder, books[index]);
        return;
      } catch (e) {
        retry--;
        if (retry == 0) {
    ...
    builder: (context) {
      return ValueListenableBuilder(
        ...
        builder: (context, value, child) {
          var isFinished = value == books.length;
          return ContentDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                LinearProgressIndicator(
                  value: value / books.length,
                ),
                const SizedBox(height: 4),
                Text("$value/${books.length}"),
                const SizedBox(height: 4),
...
Future<void> importNetworkFolder(
          ...
          return ContentDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text("Imported @a books, loaded @b pages, received @c books"
                    .tlParams({
                  "a": current,
                  "b": requestCount,
                  "c": receivedBooks,
  ...
  try {
    ...
    for (var c in books) {
      LocalFavoritesManager().addBook(resultName, c);
    }
    // 延迟一点, 让用户看清楚到底新增了多少
