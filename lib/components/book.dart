            ...
            actions: [
              Button.filled(
                onPressed: () {
                  ...
                  comicTileContext
                      .findAncestorStateOfType<_SliverGridBooksState>()!
                      .update();
                },
                child: Text('Block'.tl),
              ),
...
class SliverGridBooks extends StatefulWidget {
  const SliverGridBooks(
      {super.key,
      required this.books,
...
class _SliverGridBooksState extends State<SliverGridBooks> {
  ...
  @override
  void didUpdateWidget(covariant SliverGridBooks oldWidget) {
    if (!books.isEqualTo(widget.books)) {
      books.clear();
      for (var comic in widget.books) {
...
class _SliverGridBooks extends StatelessWidget {
  const _SliverGridBooks({
    required this.books,
  ...
  Widget build(BuildContext context) {
    return SliverGrid(
      delegate: SliverChildBuilderDelegate((context, index) {
        ...
        var badge = badgeBuilder?.call(books[index]);
        var isSelected = selection == null
            ? false
            : selection![books[index]] ?? false;
        var comic = BookTile(
          comic: books[index],
          badge: badge,
          menuOptions: menuBuilder?.call(books[index]),
          onTap: onTap != null
              ? () => onTap!(books[index], heroIDs[index])
              : null,
...
class SimpleBookTile extends StatelessWidget {
