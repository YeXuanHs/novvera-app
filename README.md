...
| 书源 | 说明 |
|------|------|
| **轻小说文库（wenku8）** | 搜索、排行、详情、目录、章节等走**官方 API**；首页推荐仅从网页抽取书籍 ID，书名/作者/封面再走 API（封面用 App 封面方案） |
| **哔哩轻小说（linovelib）** | **网页爬取**；发现 / 排行 / 搜索均只抽 ID，元数据与封面来自详情页。搜索分页有缺陷：无论点哪一页内容都相同（根因是未登录账号） |
| **幻梦轻小说（huanmeng）** | **官方 bookapi**（搜索、详情、目录、正文）；发现与分类仅爬网页拿 ID，再走 API 补全。爬虫固定桌面 UA，避免手机端拿到移动版 HTML |

### 列表页抓取约定（三源共用）

凡从**网页列表**取书（首页推荐、分类/排行 HTML、搜索 HTML），**只提取书籍 ID**。书名、作者、封面、简介等一律来自详情（官方 API 或 `/novel/{id}` / bookapi detail），不信任列表页 HTML 里的展示字段。
